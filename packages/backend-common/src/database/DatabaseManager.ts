...
import { type PluginDatabaseManager } from './types';
...
export class DatabaseManager {
  ...
  static fromConfig(
    ...
    return new DatabaseManager(config.getConfig('backend.database'), options);
  }
   ...
   *
   * @param pluginId - The plugin that the database manager should be created
   *        for. Plugin names should be unique as they are used to look up
   *        database config overrides under `backend.database.plugin`.
   */
  forPlugin(
    pluginId: string,
    deps?: {
